...
\documentclass[10pt,journal,compsoc,twoside]{TexInputs/IEEEtran}
...
\author{Woojong Koh, %~\IEEEmembership{Member,~IEEE,}
  Rahul Narain, %~\IEEEmembership{Fellow,~OSA,}
  and James F. O'Brien%~\IEEEmembership{Life~Fellow,~IEEE}%
  \IEEEcompsocitemizethanks{\IEEEcompsocthanksitem W.~Koh, R.~Narain, and J.~F.~O'Brien
    are with the Department of Electrical Engineering and Computer Sciences, University of California, Berkeley.\protect\\
% note need leading \protect in front of \\ to get a newline within \thanks as
% \\ is fragile and will error, could use \hfil\break instead.
E-mail: \{wjkoh,narain,job\}@eecs.berkeley.edu.}
%\thanks{Manuscript received April 19, 2005; revised September 17, 2014.}
}

\markboth{IEEE Transactions on Visualization and Computer Graphics}%
{Koh \MakeLowercase{\textit{et al.}}: \theTitle}
...
increase as scene complexity grows, producing a $2\times$ speed-up for a
...
\hypersetup{
  pdftitle={\theTitle},
  pdfauthor={Woojong Koh, Rahul Narain, James F. O'Brien},
  pdfsubject={\theTitle},  %<>
...
level of detail. %, whereas ours does not.
...
% Simply scaling the sizing field of visible faces with view factor according to distance and foreshortening would coarsen the mesh indiscriminately, potentially degrading the behavior of visible features.
% Instead, we impose a lower limit on the projected lengths of edges.
% This approach ensures that edges that appear too small in screen space are coarsened, while leaving visually larger edges unaffected.
% This approach takes both distance and foreshortening into account in unified and consistent way.
...
the incorrect motion of non-visible regions may cause undesirable side effects, such as producing incorrect dynamics in the stiffened regions to affect the behavior of the visible part of the cloth.
...
However, simply removing compression resistance for all non-visible faces would also create undesirable artifacts, as shown in Figure~\ref{fig:no_full_adaptive} (middle). Instead, we allow compression adaptively based on each face's
area as follows.
...
% and compute its eigendecomposition as 
% \begin{equation}
%   \textbf{E} = \textbf{Q} \begin{bmatrix}\lambda_1 & 0 \\ 0 & \lambda_2\end{bmatrix}
%   \textbf{Q}^\mathsf{T}.
% \end{equation}
...
% This area-based adaptive compensation has several advantages over non-adaptive
% compensation. First, it prevents excessive compression as shown in
%Figure~\ref{fig:no_full_adaptive} (right). On the waist of the dress, there is
%excessive compression which makes the dress shorter than the original shape.
%These overly compressed triangles inflate quickly and cause popping artifacts
%when they become visible, necessitating a much greater amount of spatial and
%temporal smoothing. Second, this excessive compression may result in extremely
%small triangles. Certain simulation steps such as collision detection and
%interpenetration handling have difficulty handling these highly compressed
%triangles robustly.
...
%if $S$ contains faces large enough to prevent buckling. Our view factor $\nu_\text{out}$
%makes faces large enough to prevent buckling because that is why we look into this
...
%Further, while the small computational overhead introduced by adaptivity is negligible for compute-intensive simulations, 
%it may be a significant cost for real-time simulation and may not outweigh the computational benefits.
...
\bibliographystyle{TexInputs/IEEEtran}
