...
\IEEEPARstart{C}{loth} simulation for visual effects has reached a mature state where
...
However, there are a few side effects: (1) slowing down simulation due to unresolved
...
refinement by spatial and temporal anticipation due to its incorrect volume, (3)
obstructing a scene and making it hard to view from other than the camera perspective, and (4) the possibility that incorrect dynamics in the stiffened regions affects the behavior of the visible part of the cloth.
...
\begin{figure}[t]
  ...
  \caption{No relaxation, full relaxation, and adaptive relaxation ($k = 10$).}
...
factor $\beta$ is unrelated to the view factor, as buckling is a physical phenomenon and
has nothing to do with view dependence. Therefore, the compression factor is defined
entirely in terms of the area of faces and the user parameter $k$.
